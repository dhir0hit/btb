...
router.get('/', function(req, res) {
  ...
  res.render('user', { title: 'BTB - '+ userName + ' ', user: userName});
});
...
router.get('/login', function(req, res, next) {
  ...
  res.render('login', { title: 'BTB - Login', user:userName });
});
...
router.post('/login', function(req, res, next) {
  ...
  collection.findOne({username: user}, {}, function (e, docs) {
    if (e) {
      ...
      if(docs.password === pass){
        ...
        res.render('login', {title: 'BTB - Login', user: "", error: "password incorrect"});
      }
    } else {
      bools = false;
    }
  ...
  collection.findOne({email: user}, {}, function (e, docs) {
    if (e) {
      ...
      if (!bools) {
        res.render('login', {title: 'BTB - Login', user: "", error: "username not found"});
      }
    }
  });
});
...
router.get('/signup', function(req, res, next) {
  ...
  res.render('signup', { title: 'BTB - Signup', user:user,  loggedIn: false});
});
...
router.post('/signup', function(req, res, next) {
  ...
  if (email) {
    ...
    res.render('signup', {title: 'BTB - Signup', user:"", loggedIn: true})
  }
  if(user) {
    ...
    collection.insert({user, Pass, profilePic, firstName, lastName, phoneNumber, Email, addressL1, addressL2, city, province, postalCode, business_account, cart}, function (error, result) {
      if(error){next(error)}
        ...
        res.render('signup', {title: 'BTB - Signup', user:req.session.username, loggedIn: false})
      }
    })
  }
});
